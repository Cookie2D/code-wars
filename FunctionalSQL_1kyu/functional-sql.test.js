describe("SQL tests", function () {
  ...
  it("Basic SELECT tests", function () {
    ...
    deepEqual(query().select().execute(), [], "No FROM clause produces empty array");
    deepEqual(query().from(numbers).execute(), numbers, "SELECT can be omited");
    deepEqual(query().execute(), []);
    deepEqual(query().from(numbers).select().execute(), numbers, "The order does not matter");
  });
  ...
  it("Basic SELECT and WHERE over objects", function () {
    ...
    deepEqual(query().select().from(persons).execute(), persons);
    ...
    deepEqual(query().select(profession).from(persons).execute(), [
      "teacher",
      "teacher",
      "teacher",
      "scientific",
    ...
    deepEqual(query().select(profession).execute(), [], "No FROM clause produces empty array");
    ...
    deepEqual(query().select(profession).from(persons).where(isTeacher).execute(), [
      "teacher",
      "teacher",
      "teacher",
    ]);
    ...
    deepEqual(query().from(persons).where(isTeacher).execute(), persons.slice(0, 3));
    ...
    deepEqual(query().select(name).from(persons).where(isTeacher).execute(), [
      "Peter",
      "Michael",
      "Peter",
    ]);
    deepEqual(query().where(isTeacher).from(persons).select(name).execute(), [
      "Peter",
      "Michael",
      "Peter",
    ]);
  ...
  it("GROUP BY tests", function () {
    ...
    deepEqual(query().select().from(persons).where(isTeacher).groupBy(profession).execute(), [
      [
        "teacher",
        [
          { name: "Peter", profession: "teacher", age: 20, maritalStatus: "married" },
    ...
    deepEqual(query().select(professionGroup).from(persons).groupBy(profession).execute(), [
      "teacher",
      "scientific",
      "politician",
    ]);
    ...
    deepEqual(query().select().from(persons).groupBy(profession, name).execute(), [
      [
        "teacher",
        [
          [
      ...
      [
        "politician",
        [["Anna", [{ name: "Anna", profession: "politician", age: 50, maritalStatus: "married" }]]],
      ],
    ]);
    ...
    deepEqual(query().select(professionCount).from(persons).groupBy(profession).execute(), [
      ["teacher", 3],
      ["scientific", 3],
      ["politician", 1],
    ]);
  ...
  it("join tests", function () {
    ...
    deepEqual(query().select(student).from(teachers, students).where(teacherJoin).execute(), [
    ...
    deepEqual(query().select().from(numbers1, numbers2).execute(), [
      [1, 4],
      [1, 5],
      [2, 4],
      [2, 5],
  ...
  it("Duplication exception tests", function () {
    function checkError(fn, clause) {
      const regexp = RegExp(`^Duplicate ${clause}$`);
      throws(fn, Error, regexp, "An error should be thrown");
    }
